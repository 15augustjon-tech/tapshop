...
// No longer generated client-side to avoid race conditions
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (buyerLat < -90 || buyerLat > 90 || buyerLng < -180 || buyerLng > 180) {
      return NextResponse.json(
        { success: false, error: 'invalid_coordinates', message: 'พิกัดที่อยู่ไม่ถูกต้อง' },
        { status: 400 }
      )
    }
    ...
    if (existingBuyer) {
      ...
      if (buyerName.trim()) {
        try {
          ...
          console.error('Update buyer name error (non-critical):', updateError)
        }
      }
    } else {
      // Create new buyer
    ...
    // Reduce stock for each product atomically using RPC to prevent race conditions
    // If any stock update fails, log error but don't rollback (manual correction if needed)
    for (const item of items) {
      try {
        ...
        if (stockError) {
          // Fall back to regular update if RPC not available
          const product = productMap.get(item.productId)!
          await supabase
            .from('products')
